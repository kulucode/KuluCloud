            ...
            if (_data.r == 0) {
                for (var i = 0; i < _data.list.length; i++) {
                    ...
                    addTableCell("truck_tab", _oneTr, "edit",
                        ...
                        + oneD.id + "','" + oneD.name + "')\"> 配置视频(" + oneD.vnum + ")</button></div>");
                }
    ...
    doRefresh(
        ...
        function (_data) {
            if (_data.r == 0) {
                $("#t_videoplip").val("");
                $("#t_videoplport").val("");
                $("#t_videopluser").val("admin");
                $("#t_videoplkey").val("888888");
                openDialogs({
                    id: "videoeditpl-base",
                    title: "批量配置视频服务",
                    width: "70%",
                ...
                $("#div_orgtrucktree").height($("#videoeditpl-base .dialog-body").height() - 54);
                iniOrgTruckTree("orgTruckTree", "div_orgtrucktree", _data.list);
            } else {
                alert(_data.error);
            }
        }
...
function iniOrgTruckTree(_name, _div, _nodes) {
    $("#" + _div).html("");
    orgTruckTree = new BSTreeView(_name, "", false, "", _div);
    orgTruckTree.imagePath = webHome + "/common/images/tree/";
    orgTruckTree.showCheck = true;
    orgTruckTree
        .addRootNode(
            "root",
            thisComp.name,
            "searchTruckFromG('','全部员工')", "", "", true);
    orgTruckTree.DrawTree(true);
    if (thisComp.id != "") {
        getOrgTruckTree(_nodes);
        orgTruckTree.getNodeById(0).doCheckBox(true);
    }
}

function getOrgTruckTree(_nodes) {
    var root = orgTruckTree.getNodeById(0);
    for (var i = 0; i < _nodes.length; i++) {
        ...
        var sub = root
            .addNode(
                ...
                true);
        if (oneD.struck > 0) {
            for (var t = 0; t < oneD.trucks.length; t++) {
                var oneT = oneD.trucks[t];
                sub.addNode(
                    oneT.id,
                    oneT.name,
                    "",
        ...
        if (oneD.cnum > 0) {
            _setChildOrgTruck(sub, oneD.children);
        }
    }
}

function _setChildOrgTruck(_pnode, _nodes) {
    for (var j = 0; j < _nodes.length; j++) {
        var oneSubD = _nodes[j];
        var subNode = _pnode.addNode(
            oneSubD.id,
            ...
            "", "&isturck=0", true);
        if (oneSubD.struck > 0) {
            for (var t = 0; t < oneSubD.trucks.length; t++) {
                var oneT = oneSubD.trucks[t];
                subNode.addNode(
                    oneT.id,
                    oneT.name,
                    "",
        ...
        if (oneSubD.cnum > 0) {
            _setChildOrgTruck(subNode, oneSubD.children);
        }
    }
}

function commitPLTruckVideo() {
    if (checkForm("videoeditpl-form") && confirm("批量配置视频服务，是否继续？")) {
        var selTruck = "";
        var selList = orgTruckTree.getCheckedNodes(false);
        for (var i = 0; i < selList.length; i++) {
            var node = selList[i];
            var _type = node.getTreeOneParaByName("isturck");
            if (_type == "1") {
                selTruck += ("," + node.name);
            }
        }
        $("#t_struckplid").val(selTruck);
        doRefresh(
            "videoeditpl-form",
            "TRUCK",
            "updatePLTruckVideo",
