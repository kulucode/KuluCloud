...
function getTruckSelectDlg(_paras, _retFun) {
    callBackFun = _retFun;
    orgTruckSelTree = null;
    getParent().$('#selecttruck-base').remove();
    var _html = "<div id=\"selecttruck-base\"><div class=\"dialog\"><div class=\"dialog-head\">";
    _html += "<span class=\"close rotate-hover\"></span> <strong></strong></div>";
    _html += "<div class=\"dialog-body\" defheight=\"0\" style=\"overflow-y:auto;padding: 1px;\"><div class=\"x4\"><div class=\"panel margin-small\"><div class=\"panel-head\">机构树</div><div class=\"panel-body padding-little\" id=\"div_truckorgseltree\"></div></div></div>";
    _html += "<div class=\"x8\"><div class=\"panel margin-small\"><div class=\"panel-head padding-little\" style=\"height:40px;\">";
    _html += "<div class=\"x5\">";
    _html += "<input type=\"hidden\" id=\"t_lookupparatruck\" name=\"t_lookuppara\"/>";
    _html += "<div class=\"x9 input-group\">";
    _html += "<input type=\"text\" style=\"width:500px;\" class=\"input input-auto\" name=\"pg_text\" id=\"pg_text2\" placeholder=\"关键词\" value=\"\"/><span class=\"addbtn\" style=\"vertical-align: top;\">";
    _html += "<input class=\"button bg-sub\" onclick=\"searchTruck(1)\" type=\"button\" value=\"搜索\"/></span>";
    _html += "</div></div></div>";
    _html += "<table id=\"seltruck_tab_h\" class=\"table table-striped table-hover table-responsive\"><tr>";
    ...
    _html += "<div style=\"overflow-y:scroll;\">";
    _html += "<table id=\"seltruck_tab\" head=\"seltruck_tab_h\" dh=\"265\" class=\"table table-striped table-hover table-responsive\">";
    _html += "<tr><td title=\"\" colspan=\"8\">加载数据……</td></tr></table>";
    _html += "</div>";
    _html += "<div id=\"seltruck_page\" class=\"panel-foot\"></div></div></div></div>";
    _html += "<div class=\"dialog-foot\">";
    _html += "<button class=\"button bg-main text-white icon-check\" onclick=\"selectCheckTruck();\"> 确认</button>";
    _html += "</div></div></div>";
    getParent().$("body").append(_html);
    $("#t_lookupparatruck").val(_paras);
    getParent().openDialogs({
        id: "selecttruck-base",
        title: "选择车辆",
        width: "80%",
    ...
    doRefresh(
        ...
        function (_data) {
            if (_data.r == 0) {
                ...
                $("#t_orgcomp").val(thisComp.id);
                searchTruck();
                iniOrgTreeByTruck("div_truckorgseltree");
            }
        });
}
...
function iniOrgTreeByTruck(_div) {
    orgTruckSelTree = null;
    $("#" + _div).html("");
    orgTruckSelTree = new BSTreeView("orgTruckSelTree", "", false, "", _div);
    orgTruckSelTree.imagePath = webHome + "/common/images/tree/";
    orgTruckSelTree
        .addRootNode(
            "root",
            thisComp.name,
            "searchTruckFromG('','全部车辆')", "", "", true);
    orgTruckSelTree.DrawTree(true);
    if (thisComp.id != "") {
        getOrgTreeByTruck();
    }
}
...
function getOrgTreeByTruck() {
    doRefresh(
        ...
        function (_data) {
            if (_data.r == 0) {
                var root = orgTruckSelTree.getNodeById(0);
                for (var i = 0; i < _data.list.length; i++) {
                    var oneD = _data.list[i];
                    var sub = root
                        .addNode(
        ...
        function (_data) {
            ...
            if (_data.r == 0) {
                for (var i = 0; i < _data.list.length; i++) {
                    ...
                    checkStr += " />";
                    var _oneTr = addTableRow("seltruck_tab");
                    addTableCell("seltruck_tab", _oneTr, "check", checkStr);
                    addTableCell("seltruck_tab", _oneTr, "nick", oneD.name + "["
                        + oneD.platenum + "]");
                }
                setPage("seltruck_page", _data.max, 50, _page, "searchTruck");
                $(document).scrollTop(0);
            }
        });
}
...
function selectCheckTruck() {
    var selRoleIds = {};
    if (confirm("是否选择车辆？")) {
        var selRole = $("#seltruck_tab input[chtype='seltruckid']:checked").each(function (i) {
            selRoleIds[$(this).attr("id")] = JSON.parse($(this).val());
        });
        if (callBackFun != null) {
            callBackFun(selRoleIds);
